...
(define (domain kitting)
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; (simulated action) collect parts and fill up the kit on the platform
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

    (:durative-action fill
        :parameters (?m - mobile ?k - kit ?s - slot ?l - part_storage)
        :duration (= ?duration (* 100 (collect-time ?k)))
        :condition (and 
            (at start (fluents_set))
            (over all (robot_at ?m ?l))
            (at start (has_arm_free ?m))
        ...
        :effect (and 
            ...
            (at end (has_arm_free ?m))
            (forall (?p - part)
                (and (at start (assign (inserted ?m ?p ?k ?s) (set-part-size ?k ?p)))))
        )
    )
